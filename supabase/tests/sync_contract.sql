...
select public.sync_push(jsonb_build_object('operation_id','20000000-0000-0000-0000-00000000000d','idempotency_key','phase271-sync-a-facility','entity_type','FACILITY_PROFILE','entity_id','40000000-0000-0000-0000-00000000000a','operation','UPDATE','local_version',1,'payload',jsonb_build_object('arabic_name','Facility A','english_name','Facility A','policy',2)));
select public.sync_push(jsonb_build_object('operation_id','20000000-0000-0000-0000-00000000000e','idempotency_key','phase271-sync-a-print','entity_type','PRINT_LOG','entity_id','50000000-0000-0000-0000-00000000000a','operation','CREATE','local_version',1,'payload',jsonb_build_object('visit_id','30000000-0000-0000-0000-00000000000a','printed_at',1)));
do $$ declare n int; c bigint; begin
 select count(*) into n from public.sync_pull(0,100); if n < 5 then raise exception 'sync_pull returned fewer than 3 changes'; end if;
